...
module Make (Syntax : Syntax.S) (Info : Info.S) = struct
  ...
  let to_template template =
    ...
    let result = Buffered.feed state `Eof in
    result |> function
    | Buffered.Done ({ len = _; buf = _; _ }, p) ->
      (*
      ...
      Ok p
    | _ ->
      print_endline "Buffered.feed error";
      Or_error.error_string "Template could not be parsed."
  ...
  let rec all ?configuration ?rule ~template ~source : Match.t list =
    ...
    let rec aux acc matches =
      ...
      | { environment; _ }::rest ->
        List.fold ~init:acc (Environment.vars environment) ~f:(fun acc v ->
            ...
            | Some source ->
              let nested_matches =
                let matches = all ?configuration ?rule ~template ~source in
